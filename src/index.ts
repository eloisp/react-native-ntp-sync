...
export default class NTPSync {
    ...
    public getAvg = () => {
        ...
        if (this.historyDetails.deltas.length >= 3) {
            // Étape 2: Trouver le delta le plus éloigné de la moyenne
            let deltaFarthestFromAvg = this.historyDetails.deltas.reduce((farthest, current) => {
                return Math.abs(current.dt - avg) > Math.abs(farthest.dt - avg) ? current : farthest;
            }, this.historyDetails.deltas[0]);

            // Étape 3: Filtrer ce delta de la liste
            let filteredDeltas = this.historyDetails.deltas.filter(delta => delta !== deltaFarthestFromAvg);

            // Étape 4: Recalculer la moyenne sans le delta le plus éloigné
            let newSum = filteredDeltas.reduce((a, b) => a + b.dt, 0);
            avg = Math.round(newSum / filteredDeltas.length) || 0
        }
